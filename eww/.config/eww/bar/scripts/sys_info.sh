...
get_wifi_ssid() {
    local ssid=$(nmcli -t -f NAME,TYPE connection show --active 2>/dev/null | grep ':802-11-wireless$' | cut -d: -f1 | head -n1)
    if [ -z "$ssid" ]; then
        echo ""
    else
        echo "$ssid"
...
get_wifi_strength() {
    local ssid=$(get_wifi_ssid)
    if [ -n "$ssid" ]; then
        # Use iw to get signal strength (more reliable than nmcli dev wifi)
        local dbm=$(iw dev wlan0 link 2>/dev/null | grep signal | awk '{print $2}')
        if [ -n "$dbm" ]; then
            # Convert dBm to percentage (rough approximation)
            # -30 dBm = 100%, -90 dBm = 0%
            local signal=$(( 100 - ((-30 - dbm) * 100 / 60) ))

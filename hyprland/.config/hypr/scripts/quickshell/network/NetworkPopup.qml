...
FloatingWindow {
        ...
        if (window.currentConn && window.currentObj) {
            if (window.activeMode === "wifi") {
                ...
                if (window.currentObj.profile) {
                    nodes.push({ id: "prof", name: window.currentObj.profile, icon: (window.currentObj.profile === "Hi-Fi (A2DP)" ? "󰓃" : "󰋎"), action: "Profil audio", isInfoNode: true, isActionable: false });
                }
    ...
    Process {
        id: btPoller
        command: ["bash", window.scriptsDir + "/bt_panel_logic.sh"]
        running: false
                        ...
                        try {
                            ...
                            if (jsonData.devices) {
                                ...
                                if (window.isBtConn && window.activeMode === "bt") {
                                    devs.push({ id: "action_settings", name: "Aktualne urządzenie", icon: "󰂯", action: "Pokaż szczegóły", isInfoNode: false, isActionable: true, cmdStr: "TOGGLE_VIEW" })
                                }
